...
if( isset($_SESSION['session']) ) {
    if( $_SESSION['session']['userLoggedIn'] === true ) {
...
try {
    ...
    if( !empty($_POST['email']) && !empty($_POST['password'])) {
        ...
        $user = new User(); // jojo
        $conditions['where'] = array('mail' => $mail);
        $conditions['return_type'] = 'single';
        $userData = $user->getRows($conditions);

        if( !$user->comparePassword( $pass, $userData['password'] ) ) {
        ...
        $sessData['theme'] = 'light';
        $_SESSION['session'] = $sessData;
